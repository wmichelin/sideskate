...
@export var rail_thickness_min: float = 1.0
@export var rail_thickness_max: float = 24.0
...
var _rail_thickness_value: Label
...
func _setup_rail_thickness_slider() -> void:
	if _body == null:
		return
	if _body.get_node_or_null("RailThicknessRow") != null:
		return
	var insert_at := 0
	var settle_row := _body.get_node_or_null("SpinLandSettleRow")
	if settle_row != null:
		insert_at = settle_row.get_index() + 1
	var row := _make_slider_row("RailThicknessRow", "rail thick", insert_at)
	_rail_thickness_value = row["value"]
	_bind_float_slider(
		row["slider"],
		rail_thickness_min,
		rail_thickness_max,
		...
		"rail_thickness",
		4.0,
		_on_rail_thickness_changed,
		_refresh_rail_thickness_label
	)
	row["slider"].focus_mode = Control.FOCUS_NONE
...
func _on_rail_thickness_changed(v: float) -> void:
	if _player != null:
		_player.set("rail_thickness", v)
	_refresh_rail_thickness_label(v)


func _refresh_rail_thickness_label(v: float) -> void:
	if _rail_thickness_value != null:
		_rail_thickness_value.text = "%.1f" % v
