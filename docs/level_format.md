...
Perspective (`perspective_inset`, `far_geometry_scale`, `reference_depth`, `reference_width`) lives on **RampLevel** / TUNING only — not in `.ssk` files. Default inset is 70; X convergence uses fixed `reference_width`. Lean is relative to the skater over `reference_depth` (not level `z_min`); drawing pads past that band so near/far perspective lines still show.
