...
**Facing** `l`/`r`: changes with world X speed when starting or reversing a grounded roll. Landing and continued coasting preserve the landed facing, including backwards riding. Spawn from `spawn_facing`.
...
**Transfer:** tap transfer when `next acid / spine` has a candidate (immediate). Holding transfer auto-fires after `transfer_hold_delay` of continuous eligibility (candidate present) — debug slider **xfer hold**, default 0.08s; delay does not arm before a candidate exists. Normal gravity. Progress is **time-phased** 0→1 from accept (never pre-seeded — that snaps lean/X). Lean is upright at the ballistic apex (`apex_frac`); if already falling, upright at mid-pull. Lateral X follows the same clock; finishes on lip touch. Keeps facing; zeros `vx` on arrival; re-anchors hang on that open edge. Takeoff `|along|` (hang stamp, else grounded `|tv.x|`, else free-air `|vx|`) rides `plan.land_along` → restored `hang_launch_along` on dest hang so remount keeps climb/approach speed (not the 120 floor). Depth (logical Z) stays free. Lean rolls start → upright → dest (never the inverted backflip half).
...
When `PlatformCaps.should_show_touch_controls()` is true — native `OS.has_feature("mobile")`, or HTML5 phone/tablet (mobile UA, iPadOS touch, or `(pointer: coarse)`) — `TouchControls` overlays an in-level virtual stick, Ollie, Transfer, and Pause. Desktop web stays keyboard-only until a real screen touch enables the pad. Joypad input hides it; a new screen touch restores it. Controls synthesize the same InputMap actions as the keyboard, with separate device ownership. Pausing, hiding, losing focus, or leaving the scene cancels virtual holds while preserving held keyboard/gamepad inputs. Main/pause menus rely on ordinary Buttons (no virtual pad).
