...
- Ordinary land: require descending support crossing; pipes only same-facing (air-out: also coping-X aligned, any height); never opposite-facing. Free-air land onto pipe/ramp maps `along = velocity.x * outward_sign()` (no forced downhill seed). Hang remount into the bowl still seeds downhill along. Air-out prefers remountable pipes; if none are under the lock, ordinary-land the nearest flat (floor/deck/lava/void) and clear hang.
